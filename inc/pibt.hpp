...
bool constraintPIBT(int curr_id, int higher_id, std::vector<State> &prev_states,
  ...
  int next[4] = {prev_loc + 1, prev_loc + grid->cols, prev_loc - 1,
                 prev_loc - grid->cols};
  ...
  // for each neighbor of (prev_loc,prev_direction), and a wait copy of current
  // location, generate a successor
  ...
  for (auto &neighbor : neighbors) {
    ...
    if (traffic[neighbor] != -1) {
      int candidates[4] = {neighbor + 1, neighbor + grid->cols, neighbor - 1,
                           neighbor - grid->cols};
  ...
  quickSort(successors, 0, successors.size() - 1, [&](State &a, State &b) {
...
#ifndef NDEBUG
  std::cout << "curr_id: " << curr_id << " prev_loc: " << prev_loc
            << " prev_orientation: " << prev_orientation << ","
            << prev_states[curr_id].timestep << std::endl;
  for (auto &next : successors) {
    std::cout << curr_id << " next: " << next.location << " " << next.timestep
              << " " << next.orientation << std::endl;
  }
  if (!heuristics.at(tasks.at(curr_id)).empty() &&
      tasks.at(curr_id) != prev_loc)
  ...
  for (auto &next : successors) {
    ...
    // next_states.at(prev_decision.at(prev_states[curr_id].location)).location
    // == -1 )
    {
      int lower_id = prev_decision.at(next.location);
      if (!constraintPIBT(lower_id, curr_id, prev_states, next_states, grid,
                          prev_decision, decision, tasks, heuristics, occupied,
                          traffic)) {
        continue;
      }
      // next_states.at(curr_id) = prev_states.at(curr_id); // Wait after
      // pushing
    }

#ifndef NDEBUG
    std::cout << "true: " << next.location << "," << next.orientation
              << std::endl;
#endif
    return true;
  }
...
#ifndef NDEBUG
  std::cout << "false: " << next_states[curr_id].location << ","
            << next_states[curr_id].orientation << std::endl;
#endif
...
inline bool moveCheck(int id, std::vector<bool> &checked,
                      std::vector<DCR> &decided, std::vector<Action> &actions,
