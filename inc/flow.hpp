...
void top_traffic(const Grid *grid, std::vector<int> &traffic, int tl_corner) {
...
void bottom_traffic(const Grid *grid, std::vector<int> &traffic,
                    int tl_corner) {
...
void left_traffic(const Grid *grid, std::vector<int> &traffic, int tl_corner) {
...
void right_traffic(const Grid *grid, std::vector<int> &traffic, int tl_corner) {
...
void build_traffic_endpoint_warehouse(const Grid *grid,
...
void build_traffic(const Grid *grid, std::vector<int> &traffic) {
  // traffic assigns valid outgoing/incoming direction(edge) for each location
  traffic.resize(grid->map.size(), -1);
  // scan for every location on each row, if a location have obstacle on top and
  // bottom but free on left and right assign right (0) to each
  // traffic[location] if have traffic set on current row assign left (2) to
  // each traffic[location] if have traffic set on next row
  int d = 0;
  bool have_rule;
  ...
  for (int i = 1; i < grid->rows - 1; i++) {
    ...
    for (int j = 1; j < grid->cols - 1; j++) {
      ...
      // if location have obstacle on top and bottom but obstacle free on left
      // and right assign d to each traffic[location]
      int candidates[4] = {location + 1, location + grid->cols, location - 1,
                           location - grid->cols};
      if ((i == 0 || grid->map[candidates[3]] == 1) &&
  ...
  // simular thing for columns but check if left and right have obstacle and top
  // and bottom are free assign down (1) to each traffic[location] if have
  // traffic set on current column assign up (3) to each traffic[location] if
  // have traffic set on next column
  d = 1;
  have_rule = false;
  for (int j = 1; j < grid->cols - 1; j++) {
    if (have_rule && d == 1) {
    ...
    for (int i = 1; i < grid->rows - 1; i++) {
      ...
      // if location have obstacle on left and right but obstacle free on top
      // and bottom assign d to each traffic[location]
      int candidates[4] = {location + 1, location + grid->cols, location - 1,
                           location - grid->cols};
      if ((j == 0 || grid->map[candidates[2]] == 1) &&
