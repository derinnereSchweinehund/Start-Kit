...
void compute_heuristics(const Grid *grid, HeuristicTable &my_heuristic,
                        int goal_location) {
  ...
  struct Node {
    ...
    Node() = default;
    Node(int location, int direction, int value)
  ...
  // initialize my_heuristic, but have error on malloc: Region cookie corrupted
  // for region
...
void compute_heuristics(const Grid *grid,
                        HeuristicTable &my_heuristic, std::vector<int> &traffic,
                        int goal_location) {
  ...
  while (!heap.empty()) {
    ...
    //     int candidates[4] = { curr.location + 1,curr.location + env->cols,
    //     curr.location - 1, curr.location - env->cols};
    ...
    for (int next : neighbors) {
      if (traffic[next] != -1) {
        ...
        if ((diff == 1 && d != 0) || (diff == -1 && d != 2) ||
            (diff == grid->cols && d != 1) || (diff == -grid->cols && d != 3))
          continue;
      }

      if (traffic[curr.location] != -1) {
        int d = traffic[curr.location];
        int candidates[4] = {curr.location + 1, curr.location + grid->cols,
                             curr.location - 1, curr.location - grid->cols};
        if (next == candidates[d])
          continue;
      }
